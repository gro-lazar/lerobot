...
class PI05Policy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        chunk is kicked off in a background thread. At ~130 ms per inference and ~33 ms per step,
        the result is ready before the queue runs dry — no hold needed at chunk boundaries.
        If inference runs over, the last action is repeated for one step (hold fallback).
        """
